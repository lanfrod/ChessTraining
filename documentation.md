...
## Описание
Данное приложение представляет собой простой не блокирующий сервер, использующий сокеты для обработки входящих соединений и запросов от клиентов. Сервер поддерживает обработку текстовых сообщений и SQL-запросов к базе данных SQLite.
...
## Основной цикл
В функции `if __name__ == '__main__':` создается серверный сокет и запускается основной цикл, который:
1. Ожидает события на сокетах.
2. Вызывает `handle_readables` для обработки входящих соединений и сообщений.
3. Вызывает `handle_writables` для отправки сообщений клиентам.

## Примечания
- Сервер использует не блокирующий режим, что позволяет обрабатывать несколько соединений одновременно.
- Обработка SQL-запросов осуществляется с помощью SQLite. Запросы должны быть корректными и безопасными.
- Не рекомендуется использовать `eval` для обработки параметров SQL-запросов, так как это может привести к уязвимостям. Лучше использовать безопасные методы передачи параметров.
